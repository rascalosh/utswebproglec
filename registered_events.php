...
if (isset($_POST['cancel_registration'])) {
    ...
    $delete_query = "DELETE FROM registrations WHERE event_id = ? AND user_id = ?";
    ...
    if ($stmt->execute()) {
        ...
        echo "Error: Could not cancel registration.";
    }
}

// Get the events that the user has registered for
$user_id = $_SESSION['user_id'];
$query = "SELECT e.* FROM events e INNER JOIN registrations r ON e.id = r.event_id WHERE r.user_id = $user_id";
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white rounded-lg shadow-lg p-6">
            ...
            <form method="post" class="mt-4">
                ...
                <button type="submit" name="cancel_registration" class="bg-red-600 text-white py-2 px-4 rounded">Cancel Registration</button>
            </form>
        </div>
        <?php } ?>
    </div>
